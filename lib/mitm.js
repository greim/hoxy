  ...
  runPhase: function(phase, transaction){
    ...
    return serialize(intercepts, function(item, next){
      ...
      if (intercept.length >= 3){
        intercept.call(item.context, transaction.request, transaction.response, next);
      } else {
        intercept.call(item.context, transaction.request, transaction.response);
        next();
      }
    });
  }
