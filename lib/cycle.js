...
export default class Cycle extends EventEmitter {
   ...
   * it has been fully piped out, but nothing received. It's up
   * to the caller of this function to call receive() on it, thus
   * getting a promise on the serverResponse object. That enables
   * hoxy to implement the 'request-send' phase where all data is
   * sent, but nothing received.
   */
  _sendToServer() {
    let req = this._request
      , resp = this._response
    ...
    return co.call(this, function*() { // return the outer promise
      ...
      if (rate > 0) { source = source.pipe(streams.brake(rate)) }
      if (latency > 0) { yield wait(latency) }
