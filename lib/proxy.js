...
module.exports = Base.extend(function(opts){
  ...
  intercept: function(opts, intercept, interceptName){
    ...
    if (!_.isEmpty(interceptName)) {
      intercept.interceptName = interceptName
    }
    this._intercepts[phase].push(intercept);
  },

  remove: function(phase, interceptName){
    ...
    if (_.isEmpty(interceptName)) {
      this.emit('log', {
            level: 'warning',
            message: 'Nothing gonna be removed without specify name'
          })
      return []
    }

    var fn = _.isFunction(interceptName) ? interceptName : function (it) { return it.interceptName === interceptName; };

    return _.remove(this._intercepts[phase], fn);
  },
