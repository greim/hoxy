...
var otherIntercept = (function(){
  ...
  return function(opts, intercept){
    var isReq = opts.phase === 'request' || opts.phase === 'sent';
    return function(req, resp, done){
      var contentType = (isReq?req:resp).headers['content-type'];
      var mimeType = contentType?contentType.replace(ctPatt, ''):undefined;
      var isMatch = 1;
      isMatch &= test(opts.contentType, contentType);
      isMatch &= test(opts.mimeType, mimeType);
      isMatch &= test(opts.port, req.port);
      isMatch &= test(opts.host, req.host);
