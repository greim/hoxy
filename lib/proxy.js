...
module.exports = Base.extend(function(opts){
  ...
  intercept: function(opts, intercept){
    ...
    if (opts.type){
      if (!typeHandlers[opts.type]){
        // TODO: test bogus type
        throw new Error('invalid type: ' + opts.type);
      }
      if (phase === 'sent' || phase === 'received'){
        // TODO: test intercept type in read only phase
        throw new Error('cannot intercept ' + opts.type + ' in ' + phase + ' phase');
      }
    }
    intercept = typeIntercept(opts, intercept); // TODO: test typeIntercept this, args, async
    intercept = filterIntercept(opts, intercept); // TODO: test filterIntercept this, args, async
    intercept = lineIntercept(opts, intercept); // TODO: test lineIntercept this, args, async
    intercept = urlIntercept(opts, intercept); // TODO: test urlIntercept this, args, async
  ...
  // TODO: get rid of this method and instead have a
  // mthod that returns a readable stream that you can
  // pipe to stdout or whatever.
  log: function(level, handler){
    if (handler){
      // TODO: test log handler
      this.on('log', handler);
    } else {
