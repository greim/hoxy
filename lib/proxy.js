...
export default class Proxy extends EventEmitter {
  ...
  constructor(opts = {}) {
    ...
    if (opts.tlsPassThrough) {
      ...
      this._tlsPassThroughServer = https.createServer({
        key,
        cert,
        SNICallback,
      }, (fromClient, toClient) => {
  ...
  listen(port) {
    ...
    if (this._tlsPassThroughServer) {
      this._tlsPassThroughServer.listen(0)
    }
    return this
  }
