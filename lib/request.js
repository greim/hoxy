...
var Request = Body.extend(function(){
  ...
  _setHttpSource: function(inReq, resolver){
    var u = inReq.url;
    if (resolver){
   ...
   */
  getDeclaredPort: function(){
    var declaredPort;
    if (this.headers.host){
      var matches = this.headers.host.match(/^[^:]+:(\d+)$/) || [];
      declaredPort =  parseInt(matches[1]);
    }
    if (!declaredPort){
      var actualPort = this.port;
      var defaultPort = this.getDefaultPort();
      if (actualPort !== defaultPort) {
        declaredPort = actualPort;
      }
    }
    return declaredPort;
  },
  ...
  getAbsoluteUrl: function(){
    var portStr = '';
    var declaredPort = this.getDeclaredPort();
    if (declaredPort) {
